 ...
 */
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgs(const std::vector<cv::Mat>& cv_img_seg,
  ...
  const int seg_channels = cv_img_seg[labelIdx].channels();
  int seg_height   = cv_img_seg[labelIdx].rows;
  int seg_width    = cv_img_seg[labelIdx].cols;

  const int data_channels = transformed_data_blobs[0].channels();
